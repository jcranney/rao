...
fn edgeslope_gaussian() {
    ...
    let measurements: Vec<rao::Measurement> = (0..100)
        .map(|idx| f64::from(idx) * 0.2)
        .map(|posx| Measurement::SlopeTwoEdge {
...
fn imat_and_cov() {
    ...
    let actuators: Vec<Actuator> = (0..100)
